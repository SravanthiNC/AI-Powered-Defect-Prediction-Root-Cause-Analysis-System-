...
# Initialize feature columns if not provided
...
def load_models():
    try:
        ...
        feature_cols = DEFAULT_FEATURE_COLS  # Use default if not found
...
if page == "Overview":
    ...
    if 'A_ERRORCODE' in main_unit_df.columns:
    ...
    if st.button("Predict Failure Probability"):
        try:
            ...
            feature_data['hour'] = [dt.hour]
            feature_data['day'] = [dt.day]
            feature_data['month'] = [dt.month]
            feature_data['day_of_week'] = [dt.weekday()]
            
            # Update encoded features
            feature_data['STAGE_encoded'] = [len(main_unit_df[main_unit_df['STAGE'] == selected_stage])]
            feature_data['VENDOR_encoded'] = [len(main_unit_df[main_unit_df['VENDOR'] == selected_vendor])]
            
            # Create DataFrame with correct column order
            features = pd.DataFrame(feature_data)[feature_cols]
            ...
            if st.checkbox("Show feature details"):
                st.write("Feature values used for prediction:", features)
                
        except Exception as e:
            st.error(f"Prediction error: {str(e)}")
            st.write("Debug information:")
            st.write("Feature columns:", feature_cols)
            st.write("Available columns:", features.columns if 'features' in locals() else "No features created yet")
